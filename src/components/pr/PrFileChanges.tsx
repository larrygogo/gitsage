...
export const PrFileChanges: Component<PrFileChangesProps> = (props) => {
  ...
  onMount(async () => {
    ...
    try {
      ...
      logger.error("PrFileChanges", "加载失败:", err);
    } finally {
      setLoading(false);
    }
  });
  ...
  const statusIcon = (status: string) => {
    switch (status) {
      case "added":
        return { text: "A", color: "#1a7f37" };
      case "removed":
        return { text: "D", color: "#cf222e" };
      case "modified":
        return { text: "M", color: "#9a6700" };
      case "renamed":
        return { text: "R", color: "#0969da" };
      default:
        return { text: "?", color: "#656d76" };
    }
  };
                    ...
                    {(item) => (
                      <div
                        style={{
                          ...
                          "background-color":
                            item.type === "add"
                              ? "var(--gs-diff-added-bg, #e6ffec)"
                              : item.type === "del"
                                ? "var(--gs-diff-deleted-bg, #ffebe9)"
                                : item.type === "header"
                                  ? "var(--gs-bg-tertiary)"
                                  : "transparent",
                          color:
