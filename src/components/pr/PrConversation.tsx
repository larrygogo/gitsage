...
type TimelineItem =
  | { type: "comment"; data: PrComment }
  | { type: "review"; data: PrReview };
...
export const PrConversation: Component<PrConversationProps> = (props) => {
  ...
  const loadTimeline = async () => {
    ...
    try {
      ...
      const timeline: TimelineItem[] = [
        ...
        ...reviews.filter((r) => r.state !== "PENDING").map((r) => ({ type: "review" as const, data: r })),
      ];
      ...
      console.error("[PrConversation] 加载失败:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitComment = async () => {
    ...
    setSubmitting(true);
    try {
      await githubService.createComment(ghRepo.owner, ghRepo.repo, props.prNumber, commentText().trim());
      setCommentText("");
      await loadTimeline();
    } catch (err) {
      console.error("[PrConversation] 发表评论失败:", err);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const reviewStateLabel = (state: string) => {
    switch (state) {
      case "APPROVED": return { text: "已批准", color: "#1a7f37", bg: "#dafbe1" };
      case "CHANGES_REQUESTED": return { text: "请求修改", color: "#cf222e", bg: "#ffebe9" };
      case "COMMENTED": return { text: "已评论", color: "#656d76", bg: "#f6f8fa" };
      default: return { text: state, color: "#656d76", bg: "#f6f8fa" };
    }
  };
    ...
    <div style={{ display: "flex", "flex-direction": "column", gap: "12px" }}>
      ...
      <Show when={ghState.currentPull?.body}>
        <div style={{
          ...
          <div style={{ "font-size": "12px", color: "var(--gs-text-muted)", "margin-bottom": "6px" }}>
                ...
                return (
                  <>
                    <div style={{ display: "flex", "align-items": "center", gap: "8px", "margin-bottom": "6px" }}>
                      ...
                      />
                      <span style={{ "font-size": "12px", "font-weight": "500", color: "var(--gs-text-primary)" }}>
                        {comment.user.login}
                      </span>
                    ...
                    <div style={{ "font-size": "13px", "white-space": "pre-wrap", "word-break": "break-word", color: "var(--gs-text-primary)" }}>
                      {comment.body}
                    </div>
                  </>
                );
                ...
                return (
                  <>
                    ...
                    <Show when={review.body}>
                      <div style={{ "font-size": "13px", "white-space": "pre-wrap", color: "var(--gs-text-primary)" }}>
                        {review.body}
                      </div>
                    </Show>
                  </>
